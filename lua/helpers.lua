...
function M.on_attach(client, bufnr)
  ...
  if client:supports_method("textDocument/rename") then
    map("n", "<leader>r", M.rename, "[r]ename")
  end
...
local diag_ns = vim.api.nvim_create_namespace("cursor_diagnostics")
 
local diag_severity_prefix = {
...
function M.show_cursor_diagnostic()
  ...
  -- pick the diagnostic whose column is closest to the cursor,
  -- breaking ties by highest severity
  local best = nil
  for _, d in ipairs(diags) do
    if best == nil then
      best = d
    else
      local d_dist    = math.abs((d.col or 0) - col)
      local best_dist = math.abs((best.col or 0) - col)
      if d_dist < best_dist then
        best = d
      elseif d_dist == best_dist and d.severity < best.severity then
        best = d
      end
    end
  end

  local prefix = diag_severity_prefix[best.severity] or ""
  local hl     = diag_severity_hl[best.severity]     or "Comment"

  vim.api.nvim_buf_set_extmark(bufnr, diag_ns, row, 0, {
    virt_text     = { { "  " .. prefix .. best.message, hl } },
    virt_text_pos = "eol",
    priority      = 200,
  })
end
